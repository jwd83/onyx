...
- **`go 1.26` directive is higher than the code needs.** Nothing here uses post-1.18 stdlib semantics; the high floor only matters for contributors on `GOTOOLCHAIN=local` with an older SDK. Lowering it is a portability courtesy, **not** an urgent fix (and explicitly *not* a blocker — the suite runs fine on the declared toolchain). Treat as optional.
