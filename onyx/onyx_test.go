...
func TestDestructiveOutputGuards(t *testing.T) {
	t.Run("preparePublic replaces only marked public directory", func(t *testing.T) {
		root := t.TempDir()
		publicDir := filepath.Join(root, "public")
		writeTestFile(t, root, "public/.onyx-generated", "old marker\n")
		writeTestFile(t, root, "public/stale.txt", "remove me\n")

		if err := preparePublic(publicDir); err != nil {
			t.Fatalf("preparePublic failed: %v", err)
		}

		if got := readTestFile(t, root, "public/.onyx-generated"); !strings.Contains(got, "generated by Onyx") {
			t.Fatalf("public marker was not recreated:\n%s", got)
		}
		if _, err := os.Stat(filepath.Join(publicDir, "stale.txt")); !os.IsNotExist(err) {
			t.Fatalf("stale public file should have been removed, err=%v", err)
		}
	})

	t.Run("preparePublic refuses file at public path", func(t *testing.T) {
		root := t.TempDir()
		publicDir := filepath.Join(root, "public")
		writeTestFile(t, root, "public", "not a directory\n")

		err := preparePublic(publicDir)
		if err == nil || !strings.Contains(err.Error(), "not a directory") {
			t.Fatalf("preparePublic error = %v, want not-a-directory refusal", err)
		}
		if got := readTestFile(t, root, "public"); got != "not a directory\n" {
			t.Fatalf("public file was modified:\n%s", got)
		}
	})

	t.Run("ensureNoJekyll creates marker when absent", func(t *testing.T) {
		root := t.TempDir()

		if err := ensureNoJekyll(root); err != nil {
			t.Fatalf("ensureNoJekyll failed: %v", err)
		}
		if got := readTestFile(t, root, ".nojekyll"); !strings.Contains(got, "Generated by Onyx") {
			t.Fatalf(".nojekyll was not created with the generated marker:\n%s", got)
		}
	})

	t.Run("ensureNoJekyll leaves existing marker alone", func(t *testing.T) {
		root := t.TempDir()
		writeTestFile(t, root, ".nojekyll", "custom\n")

		if err := ensureNoJekyll(root); err != nil {
			t.Fatalf("ensureNoJekyll failed: %v", err)
		}
		if got := readTestFile(t, root, ".nojekyll"); got != "custom\n" {
			t.Fatalf(".nojekyll should have been left unchanged:\n%s", got)
		}
	})
}
